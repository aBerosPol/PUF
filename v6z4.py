...
def volumenValjka(R, L):  # prima mm, vraća cm3
...
for i in range(len(dijametri)):
    ...
    najmanjaRazlika = 999999
    materijal = ""

    for naziv, rhoTab in gustoceMaterijala.items():
        razlika = abs(rho - rhoTab)
        if razlika < najmanjaRazlika:
            najmanjaRazlika = razlika
            materijal = naziv
            rhoTablicno = rhoTab

    #relativna pogreska AI
    relPogreska = abs(rho - rhoTablicno) / rhoTablicno * 100
    ...
    print(f"Tablična gustoća: {rhoTablicno:.2f} g/cm3")
